...
class Trainer(object):
    ...
    def print_net_params(self):
        ...
        print(params_table)
        P.print_message(
            f"Number of parameters = {all_params}, Trainable parameters = {trainable_params}"
        )
    ...
    def train_batch(self, train_data):
        ...
        if hasattr(self.net, "clip_norm"):
            nn.utils.clip_grad_norm_(self.net.parameters(), self.net.clip_norm)
    ...
    def train_epoch(self, nbatches, train_dataloader, val_dataloader):
        ...
        for idx in batch_progress_bar:
            ...
            with th.no_grad():
                ...
                self.batch_val_loss, output, target = self.forward_pass(
                    val_data, compute_loss=True
                )
                self.net.train()
    ...
    def train(self, nepochs, nbatches, train_dataloader, val_dataloader):
        ...
        epoch_progress_bar = tqdm(
            range(nepochs), desc="Epoch status: Acc=?", leave=True
        )

        for self.epoch in epoch_progress_bar:
            self.epoch_val_acc, self.epoch_val_loss = self.train_epoch(
                nbatches, train_dataloader, val_dataloader
            )
    ...
    def test(self, test_dataloader):
        ...
        with th.no_grad():
            ...
            for _ in test_progress_bar:
                ...
                val_loss, output, target = self.forward_pass(
                    test_data, compute_loss=True
                )
